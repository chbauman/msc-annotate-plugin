...
//  Copyright (C) 2023 Christian Baumann
//
...
MuseScore {
    version: "3.4"
    description: qsTr("This plugin add annotations for notes.")
    ...
    function handleChordAtCursor(cursor) {
        ...
        for (; chordIdx < allChords.length; ++chordIdx) {
            ...
            // Default: place below for voice 1 and voice 3 (numbered as 2 and 4 in user interface)
            var placement = (chord.voice & 1) ? Placement.BELOW : Placement.ABOVE;
            placement = posAbove.checked? Placement.ABOVE: (posBelow.checked? Placement.BELOW: placement)
    ...
    ColumnLayout  {
        ...
        Button {
            ...
            flat: false
            onClicked: {
                var range = getStavesRange();
                processRange(-1, -1, range[0], range[1]);
            }
            background: Rectangle {
                color: root.down? Qt.darker(btnCol, 1.2): btnCol; 
                radius: 3;
            }
        }
        ...
        CheckBox {
            ...
            text: "Enable annotations on new notes."
        }
    }
}
